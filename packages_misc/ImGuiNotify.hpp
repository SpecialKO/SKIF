...
static const ImGuiChildFlags  NOTIFY_DEFAULT_CHILD_FLAGS  = ImGuiChildFlags_AlwaysAutoResize | ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_Border;
static const ImGuiWindowFlags NOTIFY_DEFAULT_WINDOW_FLAGS = ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_Tooltip; // ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing;
...
{
private:
	ImGuiWindowFlags							flags = NOTIFY_DEFAULT_WINDOW_FLAGS;
...
public:
	 ...
	 * @param ... The variable arguments to be formatted according to the format string.
	 */
	ImGuiToast(ImGuiToastType type, int dismissTime, const char* titleLabel, const char* format, ...) : ImGuiToast(type, dismissTime)
	{
		NOTIFY_FORMAT(this->setContent, format);
 ...
 /**
	...
	inline void RenderNotifications()
	{
		...
		const float  textWrapWidth  = (600.0f * SKIF_ImGui_GlobalDPIScale < mainWindowSize.x - NOTIFY_PADDING_X * 2)
                                ?  600.0f * SKIF_ImGui_GlobalDPIScale : mainWindowSize.x - NOTIFY_PADDING_X * 2;
		...
		//const char* defaultTitle = currentToast->getDefaultTitle();
			...
			ImVec2 mainWindowPos = GetCurrentWindowRead()->Pos;
			...
			{
				currentToast->setWindowFlags(NOTIFY_DEFAULT_WINDOW_FLAGS | ImGuiWindowFlags_NoInputs);
			}

			if (BeginChild (windowName, ImVec2 (0, 0), NOTIFY_DEFAULT_CHILD_FLAGS, currentToast->getWindowFlags()))
      {
			  ...
			  {
				  PushTextWrapPos(textWrapWidth); // mainWindowSize.x / 3.f // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
